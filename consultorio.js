...
async function agendarConsulta(email) {
            ...
            <div class="Perfil-Paciente w-100">
                <div class="w-100">
                    <div class="my-2 d-flex justify-content-between align-items-center flex-wrap">
                        ...
                        <div id="updateSalud" class="w-100 flex-wrap d-none justify-content-between align-items-center mt-2">
                            <div class="row w-100 my-2 mx-auto">
                                <div class="col-4">
                                    <button onclick="mostrarParActualizar()" class="btn-main btn btn-block">Particular</button>
                                </div>
                                <div class="col-4">
                                    <button onclick="mostrarPreActualizar()" class="btn-main btn btn-block">Prepagada</button>
                                </div>
                                <div class="col-4">
                                    <button onclick="mostrarEpsActualizar()" class="btn-main btn btn-block">Eps</button>
                                </div>
                            </div>
                            <div id="contenedorEps" class="row mx-auto d-none">
                                <select id="renderSalud" class="form-control col-6 mr-1">
                                </select>
                                <button onclick="UpdateSalud()" class="btn btn-sec">Guardar</button>
                            </div>
                            <div id="contenedorPar" class="row mx-auto d-none">
                                <input id="saludParAct" class="form-control col-6 mr-1" placeholder="Particular"/>
                                <button onclick="UpdateSaludPar()" class="btn btn-sec">Guardar</button>
                            </div>
                            <div id="contenedorPre" class="row mx-auto d-none">
                                <input id="saludPreAct" class="form-control col-6 mr-1" placeholder="Prepagada"/>
                                <button onclick="UpdateSaludPre()" class="btn btn-sec">Guardar</button>
                            </div>
                        </div>
                    </div>
                    <hr>
                    <div class="row p-0">
                                ...
                                <div class="row mt-2">
                                    ...
                                    <div class="col-3 mb-2">
                                        ...
                                        <select id="horaDeConsulta" name="fechaConsulta" class="form-control">
                                        </select>
                                    </div>
                                    <div class="col-3 mb-2">
                                        <label for="horaConsulta">minuto</label>
                                        <select id="minutoConsulta" name="minutoConsulta" class="form-control">
                                        </select>
                                    </div>
                                </div>
    ...
    const horaDeConsulta = document.getElementById('horaDeConsulta');
    const minutoConsulta = document.getElementById('minutoConsulta');

    for(let cont = 0 ; cont <= 23; cont++){
            if(cont < 10){
                horaDeConsulta.innerHTML += `<option value="0${cont}">0${cont}</option>`;
            }else{
                horaDeConsulta.innerHTML += `<option value="${cont}">${cont}</option>`;
            }
    }

    for(let xx = 0; xx <= 59; xx++){
        if(xx < 10){
            minutoConsulta.innerHTML += `<option value="0${xx}">0${xx}</option>`;
        }else{
            minutoConsulta.innerHTML += `<option value="${xx}">${xx}</option>`;
        }
    }

    fechaDeConsulta.addEventListener('input', () => {
        confirmarHoraFecha(fechaDeConsulta.value, String(horaDeConsulta.value + ':' + minutoConsulta.value + ':00'));
    });
    horaDeConsulta.addEventListener('input', () => {
        confirmarHoraFecha(fechaDeConsulta.value, String(horaDeConsulta.value + ':' + minutoConsulta.value + ':00'));
    });
    minutoConsulta.addEventListener('input', () => {
        confirmarHoraFecha(fechaDeConsulta.value, String(horaDeConsulta.value + ':' + minutoConsulta.value + ':00'));
    });
}
...
async function UpdateSaludPre() {
    const datos = {
        emailPaciente: correoPac,
        salud: document.getElementById('saludPreAct').value
    }
    const peticion = await fetch('/Actualizar/salud/pre', {
        method: 'POST',
        headers: {
            'Accept': 'application/json',
            'Content-Type': 'application/json'
...
async function UpdateSaludPar() {
    const datos = {
        emailPaciente: correoPac,
        salud: document.getElementById('saludParAct').value
    }
    const peticion = await fetch('/Actualizar/salud/par', {
        method: 'POST',
        headers: {
            'Accept': 'application/json',
            'Content-Type': 'application/json'
...
function confirmarHoraFecha(fecha, hora) {
    ...
    const fechaInput = new Date(moment(fecha + 'T' + hora).format());
    const fechaActual = new Date(moment().format());
    const unaHoraDespuesActual = moment(fechaActual.getTime());
    const horaCita = moment(fechaInput);
    if (motivoConsulta.value.length >= 3) {
        console.log(horaCita);
        console.log(unaHoraDespuesActual);
...
guardarConsulta.addEventListener('click', async() => {
    ...
    switch (respuesta.estado) {
        case 'agendada':
            guardarConsulta.disabled = true;
            datosConsulta.correoDoctor = document.getElementById('mostrarOcultoCorreoDoctor').value;
...
//aca realiza la descargaingresa al modal con parametro email paciente 
...
function mostrarEpsActualizar(){
    const contenedorPre = document.getElementById('contenedorPre');
    const contenedorPar = document.getElementById('contenedorPar');
    const contenedorEps = document.getElementById('contenedorEps');

    contenedorEps.classList.add('d-flex');
    contenedorEps.classList.remove('d-none');


    contenedorPre.classList.add('d-none');
    contenedorPar.classList.add('d-none');
    contenedorPre.classList.remove('d-flex');
    contenedorPar.classList.remove('d-flex');
}

function mostrarParActualizar(){
    const contenedorPre = document.getElementById('contenedorPre');
    const contenedorPar = document.getElementById('contenedorPar');
    const contenedorEps = document.getElementById('contenedorEps');

    contenedorPar.classList.add('d-flex');
    contenedorPar.classList.remove('d-none');

    contenedorPre.classList.add('d-none');
    contenedorEps.classList.add('d-none');
    contenedorPre.classList.remove('d-flex');
    contenedorEps.classList.remove('d-flex');
}

function mostrarPreActualizar(){
    const contenedorPre = document.getElementById('contenedorPre');
    const contenedorPar = document.getElementById('contenedorPar');
    const contenedorEps = document.getElementById('contenedorEps');

    contenedorPre.classList.add('d-flex');
    contenedorPre.classList.remove('d-none');
